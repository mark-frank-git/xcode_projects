...
- (void)windowDidLoad
{
  ...
  //TODO_MSF: Not sure these are needed
  ...
  ProtocolProgram* program  = [self getCopyProgramFrom:0];
  [programsArrayController addObject:  program];
}
...
- (IBAction)saveProtocol:(id)sender
{
  ...
  protocolToBeEdited.protocolPrograms = programs;
}
...
{
  /*
  NSMutableArray *newPrograms  = [[NSMutableArray alloc] init];
  for(PatientNote *object in patient.patientListOfNotes)
  {
    [newNotes addObject:object];
  }
   */
